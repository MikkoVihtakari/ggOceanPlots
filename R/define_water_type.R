...
tmp <- lapply(1:nrow(WM), function(i) {
  ifelse(dt[[sal_col]] > WM$sal.min[i] & dt[[sal_col]] <= WM$sal.max[i] & dt[[temp_col]] > WM$temp.min[i] & dt[[temp_col]] <= WM$temp.max[i], as.character(WM$abb[i]), NA)
})

watertype <- apply(do.call(cbind, tmp), 1, function(k) {
