...
# df = as.data.frame(tmp); x = "lon"; y = "pres"; z = "temp"; format = "wide"; method = "mba"
# nx = 100; ny = 100; scale_distances = FALSE; log_y = FALSE; extrapolate = FALSE; na.rm = TRUE; return_df_names = FALSE
...
interpolate_section <- function(df = NULL, x, y, z, method = "linear", format = "ggplot", nx = 100, ny = 100, scale_distances = FALSE, log_y = FALSE, extrapolate = FALSE, na.rm = TRUE, return_df_names = FALSE) {
  ...
  if(is.null(df)) {
    ...
    if(!inherits(df, "data.frame")) stop("df has to be a data.frame")
  }
  ...
  ## Return dt if only one point for x
  
  if(length(unique(dt$x)) == 1) {
    warning("Attempting to interpolate a section with only one unique x coordinate. That won't work, sorry. Returning input data.")
    return(dt)
  }
  ...
  if(method == "linear") {
    ...
    out <- akima::interp(x = dt$x, y = dt$y, z = dt$z , xo = seq(min(dt$x), max(dt$x), length = nx), yo = seq(min(dt$y), max(dt$y), length = 100), extrap = extrapolate, duplicate = "mean")

  } else if(method == "spline") {

    out <- akima::interp(x = dt$x, y = dt$y, z = dt$z , xo = seq(min(dt$x), max(dt$x), length = nx), yo = seq(min(dt$y), max(dt$y), length = 100), extrap = extrapolate, linear = FALSE, duplicate = "mean")
  ...
  if(format == "ggplot") {
    ...
    if(method %in% c("linear", "spline")) {
      ...
      dimnames(out$xyz.est$z) <- list(out$xyz.est$x, out$xyz.est$y)
      tmp <- reshape2::melt(out$xyz.est$z, varnames = c("x", "y"), value.name = "z")
    }
